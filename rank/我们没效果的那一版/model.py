...
class RelativePositionalBias_hstu(RelativeAttentionBiasModule):
    def __init__(self, max_seq_len: int) -> None:
        super().__init__()
        # K 是每次添加的候选物品的数量
        self._max_seq_len: int = max_seq_len
        self._w = torch.nn.Parameter(
            torch.empty(2 * (2*max_seq_len + 2) - 1).normal_(mean=0, std=0.02),
        )
...
class RelativeBucketedTime_hstu(RelativeAttentionBiasModule):
    """
    ...
    def forward(
        ...
        temp = ext_timestamps[:, 1:].unsqueeze(2) - ext_timestamps[:, :-1].unsqueeze(1)
...
class HSTUMultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        # Bias modules
        if 'relative_position_bias' in attention_types or 'Dot_prd_Time_bias_Position_bias' in attention_types:
            self.relative_position_bias = RelativePositionalBias_hstu(max_len)

        if 'time_interval_bias' in attention_types or 'Dot_prd_Time_bias_Position_bias' in attention_types:
    ...
    def _rope_apply(self, x, cos, sin, rope_dim: int):
        ...
        """
        x_rope, x_pass = torch.split(x, [rope_dim, x.size(-1) - rope_dim],
                                     dim=-1)  # [B,H,S,rope_dim], [B,H,S,D-rope_dim]
    ...
    def forward(self, input, input_interval, attn_mask):
        """
        ...
        if hasattr(self, 'time_interval_bias') and input_interval is not None:
            attention_scores += self.time_interval_bias(input_interval)  # [B,1,S,S] 广播到 [B,H,S,S]
        if hasattr(self, 'relative_position_bias'):
            attention_scores += self.relative_position_bias(input_interval)  # [1,1,S,S] 广播
...
class HSTUModel(nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):
        ...
        self.itemdnn2 = nn.Linear(itemdim + args.hidden_units + userdim, args.hidden_units)
        ...
        self.click_fm = FM(input_dim=args.hidden_units * 2 + userdim + itemdim, latent_dim=args.latent_dim)
        self.expose_fm = FM(input_dim=args.hidden_units * 2 + userdim + itemdim, latent_dim=args.latent_dim)
    ...
    def _init_feat_info(self, feat_statistics, feat_types):
        ...
        self.each_embedding = {
            '103': 12*2, '104': 4*2, '105': 6*2, '109': 4*2, '106': 7*2, '107': 8*2, '108': 5*2, '110': 4*2,
            # 以下是item特征
    ...
    def forward(self, user_item, mask, action_type, seq_feature, input_interval=None):
        ...
        wide_item = origin_item_emb * item_action_mask.unsqueeze(-1)
        wide_user = origin_user_emb * item_action_mask.unsqueeze(-1)
        ...
        new_input_interval = input_interval.repeat_interleave(2, dim=1)  # [B, 2L]
        ...
        for i in range(len(self.attention_layers)):
            new_seqs, _ = self.attention_layers[i](new_seqs, new_input_interval, attention_mask)

        pred_action = new_seqs[:, ::2, :]  # Shape: [B, L, D]，Item位置是输出下一个action
        pred_action = self.last_layernorm(pred_action)
        user_sum = (pred_action * user_mask.unsqueeze(-1)).sum(dim=1, keepdim=True)  # [B,1,D]
        user_cnt = user_mask.sum(dim=1, keepdim=True).clamp_min(1).unsqueeze(-1)  # [B,1,1]
        user_mean = (user_sum / user_cnt).expand(-1, pred_action.size(1), -1)  # [B,L,D]

        pred_input = torch.cat([pred_action, user_mean, wide_item, wide_user], dim=-1)  # [B, L, 2D+item_dim+user_dim]

        pred_click = self.click_fm(pred_input)  # [B, L, 1]
        pred_expose = self.expose_fm(pred_input)
        pred_click_expose = torch.cat([pred_expose, pred_click], dim=-1)

        rank_loss_mask = (mask == 1).to(self.dev)  # [B, L]
        flat_mask = rank_loss_mask.view(-1).bool()  # [B*L]

        labels = action_type.to(self.dev)  # [B, L]
        valid_logits = pred_click_expose.view(-1, 2)[flat_mask]  # [B*L, 1]
        valid_labels = labels.view(-1)[flat_mask].long()  # [B*L]

        all_valid_logits = valid_logits
        all_valid_labels = valid_labels

        # ---------- 负样本随机下采样：neg:pos ≈ n:1 ----------
        n = 3.0
        pos_idx = torch.nonzero(all_valid_labels == 1, as_tuple=False).squeeze(1)
        neg_idx = torch.nonzero(all_valid_labels == 0, as_tuple=False).squeeze(1)
        if pos_idx.numel() > 0 and neg_idx.numel() > 0:
            num_pos = min(pos_idx.numel()//2, pos_idx.numel())  # 正样本抽取一半
            perm_pos = torch.randperm(pos_idx.numel(), device=pos_idx.device)[:num_pos]
            num_neg = min(neg_idx.numel(), int(n * num_pos))  # 负样本为正样本的n倍
            perm_neg = torch.randperm(neg_idx.numel(), device=neg_idx.device)[:num_neg]
            keep = torch.cat([pos_idx[perm_pos], neg_idx[perm_neg]], dim=0)
            valid_logits = all_valid_logits[keep]
            valid_labels = all_valid_labels[keep]

        # ---------- CE 计算 ----------
        class_weights = torch.tensor([1, n], device=self.dev, dtype=valid_logits.dtype)
        loss_fun_rank = nn.CrossEntropyLoss(weight=class_weights)
        rank_loss = loss_fun_rank(valid_logits.float(), valid_labels)

        with torch.no_grad():
            p = F.softmax(valid_logits, dim=1)[:, 1]
            self.auroc.update(p.detach(), valid_labels.detach())
    ...
    def score_candidates(self, user_item, mask, action_type, seq_feature, input_interval=None, candidate_seq=None,
        ...
        positions_cand = torch.full((B, K), fill_value=2*L-1, device=device, dtype=torch.long)  # 所有候选物复用最后一个item的position embedding
