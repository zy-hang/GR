...
class HSTUModel(nn.Module):
    ...
    def forward(self, user_item, mask, action_type, seq_feature, input_interval=None):
        ...
        labels = action_type.to(self.dev).float()  # [B, L] in {0,1}

        valid_logits = pred_click.view(-1)[flat_mask]  # [N]
        valid_labels = labels.view(-1)[flat_mask]  # [N] float{0,1}

        # ------- 负样本下采样（neg:pos≈n:1）-------
        n = 3.0  # 可按需调节
        if valid_labels.numel() > 0:
            pos_idx = torch.nonzero(valid_labels == 1, as_tuple=False).squeeze(1)
            neg_idx = torch.nonzero(valid_labels == 0, as_tuple=False).squeeze(1)
            if pos_idx.numel() > 0 and neg_idx.numel() > 0:
                num_pos = max(1, pos_idx.numel())
                perm_pos = torch.randperm(pos_idx.numel(), device=device)[:num_pos]
                num_neg = min(neg_idx.numel(), int(n * num_pos))
                perm_neg = torch.randperm(neg_idx.numel(), device=device)[:num_neg]
                keep = torch.cat([pos_idx[perm_pos], neg_idx[perm_neg]], dim=0)
                valid_logits = valid_logits[keep]
                valid_labels = valid_labels[keep]

        # ------- BCEWithLogits（带 pos_weight）-------
        num_pos = valid_labels.round().sum().clamp_min(1.0)
        num_all = torch.tensor(valid_labels.numel(), device=device, dtype=valid_labels.dtype).clamp_min(1.0)
        num_neg = (num_all - num_pos).clamp_min(1.0)
        pos_weight = (num_neg / num_pos).to(valid_logits.dtype)

        rank_loss = F.binary_cross_entropy_with_logits(
            valid_logits.float(), valid_labels, pos_weight=pos_weight
        )

        # ------- AUC（点击头）-------
        with torch.no_grad():
            # 若本批只有单类，AUC 退化为 0.0，避免报错
            uniq = torch.unique(valid_labels.round())
            if uniq.numel() == 2:
                p = torch.sigmoid(valid_logits)
                self.auroc.update(p.detach(), valid_labels.round().long())
