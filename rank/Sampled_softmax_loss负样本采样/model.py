...
class HSTUModel(nn.Module):
    ...
    def forward(self, user_item, mask, action_type, seq_feature, input_interval=None, neg_ids=None, neg_feat=None):
        ...
        history_seqs = new_seqs.clone()
        positions_hist = positions  # [B, 2L]
        ltr_hist = ltr_pos  # [B, 2L]
        hist_pos_mask = (new_log_seqs != 0)  # [B, 2L], bool
        ...
        masked_logits = logits_mat.masked_fill(~class_valid, NEG_INF)
        pos_logit_base = masked_logits[:, :, 0]  # [B, L]
        lse_total = torch.logsumexp(masked_logits, dim=2)  # [B, L]
        row_valid_all = row_valid & valid_user.view(-1, 1)  # [B, L]

        # ===== NEW: neg-pool（复用 batch 的 neg / neg_feat） =====
        if (neg_ids is not None) and (neg_feat is not None):

            # 只取 item 位置且有有效 neg 的位置
            pool_pos = (mask == 1) & (neg_ids > 0)  # [B, L]
            if pool_pos.any():
                # 展平成 1 维索引
                flat_idx = pool_pos.view(-1).nonzero(as_tuple=False).squeeze(1)  # [F]
                # 候选 id 序列 [1, F]
                cand_ids = neg_ids.view(-1)[flat_idx].unsqueeze(0)

                # 把每个特征也按相同位置取出来组成 [1, F, ...]
                cand_feat = {}
                for k, t in neg_feat.items():
                    # t: [B, L, ...] -> [B*L, ...] -> gather -> [F, ...] -> [1, F, ...]
                    t2 = t.view(-1, *t.shape[2:])
                    cand_feat[k] = t2.index_select(0, flat_idx).unsqueeze(0)

                # 分块，把候选拼到历史后面跑注意力，并把它们的 logits 合进分母
                Fall = cand_ids.shape[1]
                chunk = int(self.neg_pool_chunk)

                for s in range(0, Fall, chunk):
                    e = min(s + chunk, Fall)
                    K = e - s
                    # 当前候选块
                    cand_seq_blk = cand_ids[:, s:e]  # [1, K]
                    cand_feat_blk = {k: v[:, s:e, ...] for k, v in cand_feat.items()}

                    # 候选 embedding（只含 item）
                    cand_emb, origin_cand_emb, _ = self.feat2emb(cand_seq_blk, cand_feat_blk, include_user=False)  # [1, K, D]
                    cand_emb = cand_emb.expand(B, -1, -1).contiguous()  # [B, K, D]
                    cand_item_forward = self.itemdnn_forward(origin_cand_emb)  # [1, K, D]
                    cand_item_forward = cand_item_forward.expand(B, -1, -1).contiguous()  # [B, K, D]

                    # 拼接 & 位置嵌入（候选复用最后一个 item 的 pos / ltr）
                    x = torch.cat([history_seqs, cand_emb], dim=1)  # [B, 2L+K, D]

                    # 候选统一复用“最后一个历史 item 的绝对位置”（2L-1）or -2, 只给“候选部分”补充位置与右位置信息；历史部分不再二次添加
                    positions_cand = torch.full((B, K), fill_value=2 * L - 1, device=device, dtype=torch.long)
                    cand_ltr = ltr_hist[:, -2].unsqueeze(1).expand(-1, K)  # [B, K]
                    x[:, 2 * L:, :] = x[:, 2 * L:, :] + self.pos_emb(positions_cand) + self.pos_emb_right(cand_ltr)

                    # M-FALCON 掩码：历史因果 + 候选自见
                    total_len = 2 * L + K
                    m = torch.ones((total_len, total_len), dtype=torch.bool, device=device)
                    m = torch.tril(m)
                    m[2 * L:, 2 * L:] = torch.eye(K, dtype=torch.bool, device=device)
                    # 可见位置掩码：历史部分按 hist_pos_mask，候选全部可见
                    pos_mask_all = torch.cat(
                        [hist_pos_mask, torch.ones((B, K), dtype=torch.bool, device=device)], dim=1
                    )  # [B, 2L+K]
                    final_mask = m.unsqueeze(0) & pos_mask_all.unsqueeze(1) & pos_mask_all.unsqueeze(2)  # [B,2L+K,2L+K]

                    # 过注意力层
                    for layer in self.attention_layers:
                        x, _ = layer(x, final_mask)
                    x = self.last_layernorm(x)  # [B, 2L+K, D]

                    cand_user_foward = x[:, 2 * L:, :]  # [B, K, D]
                    cand_predict = cand_user_foward * cand_item_forward  # [B, K, D]
                    logits_g = self.click_fm(cand_predict).squeeze(-1) / float(getattr(self, "t", 0.07))   # [B, K]

                    # 把候选块并入各点击行的分母（对 K 做 logsumexp，按行广播到 [B, L]）
                    lse_g = torch.logsumexp(logits_g, dim=1)  # [B]
                    lse_total = torch.logaddexp(lse_total, lse_g.unsqueeze(1))  # [B, L]

        # === 汇总 loss（保持原逻辑） ===
        per_row_loss = (-pos_logit_base + lse_total) * row_valid_all.float()
        loss_sum_per_user = per_row_loss.sum(dim=1)
        num_rows_per_user = row_valid_all.sum(dim=1).clamp_min(1)
        mean_loss_per_user = loss_sum_per_user / num_rows_per_user
        rank_loss = (mean_loss_per_user * valid_user.float()).sum() / valid_user.float().sum().clamp_min(1.0)

        # AUC（保持原逻辑）
        with torch.no_grad():
            eval_mask = item_pos_mask & valid_user.view(-1, 1)
            v_logits = logits_all[eval_mask]
            v_labels = labels_all.float()[eval_mask]
        ...
        diagnostics = {'rank_loss': rank_loss, 'auc': auc}
