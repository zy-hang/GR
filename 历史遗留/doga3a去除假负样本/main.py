...
def train_model(args, log_file, writer, data_path):
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step, batch in enumerate(train_loader):
            ...
            loss, (all_remove_neg_id,all_v_neg_id) = get_loss_streaming(
                args, pos,neg,pos_logits, (q_valid, neg_cache, model, neg_bank_cur), next_token_type, next_action_type, action_only=False
            )
            if args.l2_emb > 0:
            ...
            if do_step and (global_step % out_put_step == 0):
                ...
                print('当前step移除了重复负样本ID数量（数量/总负样本ID数）:', all_remove_neg_id, '/', all_v_neg_id,
                      f'，比例={all_remove_neg_id/all_v_neg_id:.6f}')
                T_since_log = 0.0
                T_total_since_log_min = 0.0
            t0 = time.time()
        t1_epoch = time.time()
