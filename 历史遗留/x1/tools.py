...
def get_args():
    ...
    parser.add_argument('--ckpt_every', type=int, default=6,
...
def get_loss_streaming(args, pos_logits, neg_bank_tuple,
    ...
    # ========= NEW: 位置权重（对每个用户的可预测位 0.5 → 1.5 线性递增）=========
    # - 对每个 batch 内的用户单独计算：该用户的第1个预测位权重=0.5，最后一个=1.5
    # - 中间线性插值；若该用户只有1个预测位，则权重=1.0（避免除0，且合理）
    with torch.no_grad():
        a = float(getattr(args, 'pos_weight_a', 0.5))   # 新增：位置权重跨度 a

        B, L = next_token_type.shape
        mask_all_f = mask_all.to(torch.float32)               # [B,L]
        rank = (mask_all_f.cumsum(dim=1) - mask_all_f)        # [B,L] 0..M_b-1
        count = mask_all_f.sum(dim=1, keepdim=True)           # [B,1] M_b
        denom = torch.clamp(count - 1.0, min=1.0)             # [B,1] M_b-1，避免除0
        ratio = rank / denom                                  # [B,L] 0→1

        # 线性映射到 [1-a, 1+a]
        weights_full = (1.0 - a) + (2.0 * a) * ratio          # [B,L]

        # 可选：若某用户只有 1 个预测位，让该位权重=1（居中更合理）
    ...
    for st in range(0, bank.size(0), chunk):
        bk = bank[st:st + chunk, :].to(q_valid.dtype)            # [D, Chunk]
        s = (q_valid @ bk.T).to(torch.float32) * scale              # [M, D] @ [D, Chunk] = [M, Chunk]
    ...
    # 加权均值
    w_all = weights_valid
    loss_all = (loss_vec * w_all).sum() / w_all.sum()

    if click_mask_valid.any():
        w_click = w_all[click_mask_valid]
        loss_click = (loss_vec[click_mask_valid] * w_click).sum() / w_click.sum()
    else:
        loss_click = loss_all.detach()*0.0

    loss = loss_click if action_only else (0.2 * loss_click + loss_all)
