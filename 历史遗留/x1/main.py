...
def train_model(args, log_file, writer, data_path):
    ...
    # 查看当前的USER_CACHE_PATH下的已经保存的任务，如果任务数量大于4个，则删除最早的任务
    clear_cache(cache_dir)

    task_id, instance_id = parse_job_ids_from_path(Path(os.getenv('TRAIN_CKPT_PATH', './')))
    cache_dir = cache_dir + f"/task{task_id}_inst{instance_id}"

    if args.state_dict_path != None:
        load_path = Path(cache_dir,f'global_step_{global_step}_epoch_{epoch_start_idx-1}')
        meta = load_epoch_checkpoint(ckpt_dir=load_path, optimizer=optimizer, scheduler=scheduler, device=args.device)
        if meta['rng_states'] is not None:
            torch.set_rng_state(meta['rng_states']['torch'])
            np.random.set_state(meta['rng_states']['numpy'])
            random.setstate(meta['rng_states']['python'])
        if meta['dataloader_rng_state'] is not None:
            # 恢复状态前跑一个 next()，让状态前进一位
            _ = next(iter(train_loader))
            train_loader.sampler.generator.set_state(meta['dataloader_rng_state'])
        print(f"Resumed from checkpoint: {args.state_dict_path}, epoch_start_idx={epoch_start_idx}, global_step={global_step}")
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        if args.inference_only:
            break
        t0 = time.time()
        micro_in_this_epoch = 0  # 本 epoch 内的微步数量
        ...
        for step, batch in enumerate(train_loader):
            ...
            with autocast(dtype=torch.bfloat16, device_type='cuda', enabled=True):
                pos_logits, _, neg_bank_tuple = model(
            ...
            if (neg_cache is not None) and (len(neg_cache) > 0):
                for neg_cached, neg_feat_cached, next_token_type_cached in neg_cache:
                    if args.neg_cache_on_cpu:
                        ...
                        neg_feat_cached = {k: v.to(args.device, non_blocking=True) for k, v in neg_feat_cached.items()}
                    # ⚠️ 重算历史负样本的 embedding（使用当前模型参数）
                    with autocast(dtype=torch.bfloat16, device_type='cuda', enabled=True):
                        neg_embs_hist = model.feat2emb(neg_cached, neg_feat_cached, include_user=False)
                        neg_embs_hist = neg_embs_hist / neg_embs_hist.norm(dim=-1, keepdim=True).clamp_min(1e-8)
                    hist_mask = (next_token_type_cached == 1).view(-1)
                    neg_embs_hist = neg_embs_hist.view(-1, neg_embs_hist.size(-1))[hist_mask]
                    if neg_embs_hist.numel() > 0:
                        extra_banks.append(neg_embs_hist)
            big_neg_bank = torch.cat(extra_banks + [neg_bank_cur], dim=0) if len(extra_banks) > 0 else neg_bank_cur
            ...
            loss, loss_click, lose_expose, pos_mean_click, neg_mean_click, pos_mean_all, neg_mean_all = get_loss_streaming(
                args, pos_logits, (q_valid, big_neg_bank), next_token_type, next_action_type, action_only=False
            )
            if args.l2_emb > 0:
        ...
        rng_states = dict(torch=torch.get_rng_state(),
                          numpy=np.random.get_state(),
                          python=random.getstate())

        save_path = Path(cache_dir,f'global_step_{global_step}_epoch_{epoch}')
        # 清理旧检查点，只保留最新的2个
        if epoch >= 3:
            old_step = old_global_step.popleft()
            print(f"Removing old checkpoint at epoch {epoch-2}")
            old_ckpt_path = Path(cache_dir, f'global_step_{old_step}_epoch_{epoch-2}')
            print(f"Old checkpoint path: {old_ckpt_path}")
            if old_ckpt_path.exists():
                print(f"Removing old checkpoint dir {old_ckpt_path}")
                for file in old_ckpt_path.iterdir():
                    file.unlink()
                old_ckpt_path.rmdir()
                print(f"Old checkpoint dir {old_ckpt_path} removed.")
        old_global_step.append(global_step)
        save_epoch_checkpoint(save_path, optimizer, scheduler, rng_states)
