...
def process_cold_start_feat(feat: dict, testdataset = None):
    processed_feat = {}
    for feat_id, feat_value in feat.items():
        if isinstance(feat_value, list):
            processed_feat[feat_id] = [testdataset.feat_top3_dict.get(feat_id, [0])[0] if isinstance(v, str) else v for v in feat_value]
        elif isinstance(feat_value, str):
            processed_feat[feat_id] = testdataset.feat_top3_dict.get(feat_id, [0])[0]
        else:
...
def build_candidates(testdataset, model):
    ...
    with open(candidate_path, 'r') as f:
        for line in f:
            ...
            # testdataset._inject_dynamic_item_feats(item_id, feature)
            feature = process_cold_start_feat(feature, testdataset)
        ...
        with torch.autocast('cuda', dtype=torch.bfloat16):
            ...
            for step, batch in enumerate(test_loader):
                if args.load_time_intervals:
                    seq, token_type, action_type, seq_feat_user, seq_feat_item, user_id, cur_timestamp, target_actions = batch
                    def to_dev(d):
                    ...
                    logits = model.predict(seq, seq_feat_user, seq_feat_item,token_type, action_type, input_interval,target_actions)
                    # 记录历史序列
                    for s in seq:
