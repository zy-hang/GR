...
@torch.autocast('cuda', enabled=True, dtype=torch.bfloat16)
def get_loss_streaming_v2(args, pos_logits, neg_bank_tuple,
                       next_token_type, next_action_type,
                       action_only=False, temperature=0.022):
    ...
    chunk = 4096
    M = q_valid.size(0)
    mchunk = min(13312, M) # q_valid方向切块

    def _blk(q_tile, bk):
        with autocast(device_type='cuda', dtype=torch.bfloat16, enabled=True):
            s = q_tile @ bk.T
        return (s.to(torch.float32) * scale).logsumexp(dim=1)
    ...
    if neg_cache is not None and len(neg_cache) > 0:
        for neg_cached, neg_feat_cached, next_token_type_cached in neg_cache:
            ...
            with autocast(dtype=torch.bfloat16, device_type='cuda', enabled=True):
                ...
                neg_embs_hist = F.normalize(neg_embs_hist.float(), dim=-1, eps=1e-8).to(neg_embs_hist.dtype)
            '''
            def _embed_hist(neg_cached):
                with autocast(dtype=torch.bfloat16, device_type='cuda', enabled=True):
                    out = model.feat2emb(neg_cached, neg_feat_cached, include_user=False)
                    out = F.normalize(out.float(), dim=-1, eps=1e-8).to(out.dtype)
                return out
            neg_embs_hist = checkpoint(_embed_hist, neg_cached, use_reentrant=False)
            ...
            for st in range(0, bank.size(0), chunk):
                bk = bank[st:st + chunk, :]
                for ms in range(0, q_valid.size(0), mchunk):
                    me = min(ms + mchunk, q_valid.size(0))
                    add = checkpoint(_blk, q_valid[ms:me, :], bk, use_reentrant=False)  
                    lse[ms:me] = torch.logaddexp(lse[ms:me], add)
                del bk
