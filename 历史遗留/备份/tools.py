...
def get_args():
    ...
    parser.add_argument('--clip_norm', default=2.0, type=float)
    ...
    # 梯度累积步速
    parser.add_argument('--grad_accum_steps', type=int, default=2, help='梯度累积的累计步数')

    # 负样本缓存（跨 step 的 memory bank）
    parser.add_argument('--neg_cache_steps', type=int, default=5,
    ...
    parser.add_argument('--state_dict_path', default='/apdcephfs_fsgm/share_303710656/angel/ams_2025_1029731852466342281/angel_training_ams_2025_1029731852466342281_20251022133110_f8195492/self/95a8cce6994cdb97019a0cbf347d142e/ckpt/global_step26408_epoch4_loss4.8204/model.pt', type=str)
...
def get_loss_streaming(args, pos_logits, neg_bank_tuple,
                       ...
                       action_only=False, temperature=0.018, pos_ids_valid=None, bank_ids=None):
    ...
    q_valid, neg_cache, model, neg_bank_cur, neg_ids_cur = neg_bank_tuple
    ...
    chunk = 4096
    def _blk(q_tile, bk, pos_ids_valid, ids_chunk):
        with autocast(device_type='cuda', dtype=torch.bfloat16, enabled=True):
            s = q_tile @ bk.T
        s = s.to(torch.float32) * scale
        with torch.no_grad():
            mask_same = (pos_ids_valid.view(-1, 1) == ids_chunk.view(1, -1))  # [M, c]
    ...
    if neg_cache is not None and len(neg_cache) > 0:
        for neg_cached, neg_feat_cached, next_token_type_cached in neg_cache:
            ...
            ids_bank = neg_cached.view(-1)[hist_mask]
            for st in range(0, bank.size(0), chunk):
                ed = min(st + chunk, bank.size(0))
                bk = bank[st:ed, :]
                ids_chunk = ids_bank[st:ed]
                s = checkpoint(_blk, q_valid, bk, pos_ids_valid, ids_chunk, use_reentrant=False) 
                lse = torch.logaddexp(lse, s)
                del s, bk
            del neg_cached, neg_feat_cached, next_token_type_cached, hist_mask, neg_embs_hist, bank
            torch.cuda.empty_cache()

    # 处理当前步的负样本
    del neg_cache
    bank = neg_bank_cur.to(q_valid.dtype)
    ids_bank = neg_ids_cur
    for st in range(0, bank.size(0), chunk):
        ed = min(st + chunk, bank.size(0))
        bk = bank[st:ed, :]
        ids_chunk = ids_bank[st:ed]
