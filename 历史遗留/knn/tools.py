...
def get_args():
    ...
    parser.add_argument('--neg_cache_steps', type=int, default=8,
    ...
    parser.add_argument('--knn_num', default=50, type=int, help='Number of KNN neighbors for cold start items')
...
def get_loss_streaming(args, pos_logits, neg_bank_tuple,
    ...
    if neg_cache is not None and len(neg_cache) > 0:
        for neg_cached, neg_feat_cached, next_token_type_cached, cold_start_tensor_neg in neg_cache:
            ...
            with autocast(dtype=torch.bfloat16, device_type='cuda', enabled=True):
                neg_embs_hist = model.feat2emb(neg_cached, neg_feat_cached, include_user=False,cold_start_list_tensor=cold_start_tensor_neg)
            ...
            del neg_cached, neg_feat_cached, next_token_type_cached, hist_mask, neg_embs_hist
    ...
    loss = loss_click if action_only else (0.2*loss_click + 0.3*loss_conv + loss_all)
...
def fill_neg_cache(args, train_loader, neg_cache):
    if neg_cache is not None and len(neg_cache) < args.neg_cache_steps:
        ...
        for step, batch in enumerate(train_loader):
            neg = batch[2]
            next_token_type = batch[5]
            neg_feat = batch[10]
            cold_start_list_tensor = batch[15]  # [B, L, K]
            def to_dev(d):
            ...
            neg_cache.append((neg_store, neg_feat_store, next_token_type_store, cold_start_list_tensor))
...
def output_info(global_step, writer,outpot_dice,is_train=True):
    """
...
def grad_total_norm(parameters, norm_type: float = 2.0) -> torch.Tensor:
    # 计算当前梯度的总范数（与 PyTorch 实现一致的思路）
    ...
    for p in parameters:
        ...
        if g.is_sparse:
            g = g.coalesce()
            values = g._values()
            n = values.abs().max() if norm_type == float('inf') else values.float().norm(norm_type)
        else:
            n = g.abs().max() if norm_type == float('inf') else g.float().norm(norm_type)
        norms.append(n)
    if not norms:
        return torch.tensor(0.0, device=next(iter(parameters)).device if isinstance(parameters, list) and parameters else 'cpu')
