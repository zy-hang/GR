...
class HSTUModel(nn.Module):
    ...
    def __init__(self, user_num, item_num, remap_item_num, feat_statistics, feat_types, args):
        super().__init__()
        ...
        # item Embedding
        self.item_emb = nn.Embedding(remap_item_num + 1, 64, padding_idx=0, dtype=torch.bfloat16)
        self.item_emb_ = nn.Embedding(remap_item_num + 1, 64, padding_idx=0, dtype=torch.bfloat16)
        
        # position embedding
        self.pos_emb = nn.Embedding(2 * args.maxlen + 1, int(args.hidden_units//4), padding_idx=0, dtype=torch.bfloat16)
        self.pos_emb_right = nn.Embedding(2 * args.maxlen + 1, int(args.hidden_units//4), padding_idx=0, dtype=torch.bfloat16)
        self.action_emb = torch.nn.Embedding(3 + 1, int(args.hidden_units//4), padding_idx=0, dtype=torch.bfloat16)
        self.action_position_embedding = nn.Linear(int(args.hidden_units//4)*3, args.hidden_units)

        # action embedding (sparse featrue)
        self.action_emb_append = torch.nn.Embedding(3 + 1, 16, padding_idx=0, dtype=torch.bfloat16)
        ...
        for k in self.USER_SPARSE_FEAT:
            self.sparse_emb[k] = nn.Embedding(self.USER_SPARSE_FEAT[k] + 1, self.each_embedding[k], padding_idx=0, dtype=torch.bfloat16)
        for k in self.ITEM_SPARSE_FEAT:
            self.sparse_emb[k] = nn.Embedding(self.ITEM_SPARSE_FEAT[k] + 1, self.each_embedding[k], padding_idx=0, dtype=torch.bfloat16)
        for k in self.ITEM_ARRAY_FEAT:
            self.sparse_emb[k] = nn.Embedding(self.ITEM_ARRAY_FEAT[k] + 1, self.each_embedding[k], padding_idx=0, dtype=torch.bfloat16)
        for k in self.USER_ARRAY_FEAT:
            self.sparse_emb[k] = nn.Embedding(self.USER_ARRAY_FEAT[k] + 1, self.each_embedding[k], padding_idx=0, dtype=torch.bfloat16)
        for k in self.ITEM_EMB_FEAT:
            self.emb_transform[k] = nn.Linear(self.ITEM_EMB_FEAT[k], 32)

        # 新增：时间离散特征（仅历史分支会使用）
        for k in self.ITEM_TIME_SPARSE_FEAT:
            self.sparse_emb[k] = nn.Embedding(self.ITEM_TIME_SPARSE_FEAT[k] + 1, self.each_embedding[k], padding_idx=0, dtype=torch.bfloat16)

        # 新增：物品动态特征
        for k in self.ITEM_DYNAMIC_FEAT:
            self.sparse_emb[k] = nn.Embedding(self.ITEM_DYNAMIC_FEAT[k] + 1, self.each_embedding[k], padding_idx=0, dtype=torch.bfloat16)
    ...
    @dynamo.disable
    def _apply_coldstart_replace(self, item_embedding, mask_for_neg, cold_start_list_tensor):
        # 将-1位置的embedding，使用冷启动得到的邻居emb的均值来替换
        if (cold_start_list_tensor is not None 
            and isinstance(cold_start_list_tensor, torch.Tensor) 
            and cold_start_list_tensor.numel() > 0):
            # 期望形状: [B, S, K]，0 表示 padding
            knn_tensor = cold_start_list_tensor.to(self.dev).long()
            dim = item_embedding.size(-1)

            flat_mask = mask_for_neg.view(-1)                 # [B*S]
            if knn_tensor.numel() > 0:
                valid_mask = (knn_tensor != 0).unsqueeze(-1)    # [M, K, 1]
                emb1 = self.item_emb(knn_tensor)                # [M, K, 64]
                emb2 = self.item_emb_(knn_tensor)               # [M, K, 64]
                emb = torch.cat([emb1, emb2], dim=-1)         # [M, K, 128]
                emb = emb * valid_mask
                denom = valid_mask.sum(dim=1).clamp_min(1).to(emb.dtype)  # [M,1]
                mean_emb = emb.sum(dim=1) / denom             # [M, 128]

                item_embedding.view(-1, dim)[flat_mask] = mean_emb
        return item_embedding
    # --------- 聚合 item / user 的特征 ----------
    def _gather_item_feats(self, seq, feature_tensors, include_time: bool,cold_start_list_tensor):
        # item_embedding = torch.cat([self.item_emb(seq), self.item_emb_(seq)], dim=-1)
        # item_feat_list = [item_embedding]

        mask_for_neg = (seq == -1)
        seq = seq.masked_fill(mask_for_neg, 0)
        item_embedding = torch.cat([self.item_emb(seq), self.item_emb_(seq)], dim=-1)

        item_embedding = self._apply_coldstart_replace(item_embedding, mask_for_neg, cold_start_list_tensor)
    ...
    def feat2emb(self, seq, feature_tensors, mask=None, include_user=False,action_type=None,cold_start_list_tensor=None):
        """
        ...
        _, item_feat_list = self._gather_item_feats(item_seq, feature_item, 
        include_time=include_user,cold_start_list_tensor=cold_start_list_tensor)
    ...
    def log2feats(self, log_seqs, mask, seq_user_feature,seq_item_feature,input_interval=None,
                   action_type=None, next_action_type=None, next_mask=None,cold_start_list_tensor=None):
        
        batch_size = log_seqs.shape[0]
        maxlen = log_seqs.shape[1]

        # Get sequence embeddings
        seqs = self.feat2emb(log_seqs, (seq_user_feature,seq_item_feature), 
                             mask=mask, include_user=True,action_type=action_type,cold_start_list_tensor=cold_start_list_tensor)  # [B, L, D]
    ...
    def forward(
        ...
        cold_start_list_tensor_seq=None, cold_start_list_tensor_pos=None, cold_start_list_tensor_neg=None
    ):
        """Forward pass for training"""
        log_feats = self.log2feats(user_item, mask, seq_user_feature,seq_item_feature,
                                    input_interval, action_type, next_action_type, next_mask,
                                    cold_start_list_tensor_seq)  # [B, L, D]
        
        loss_mask = (next_mask == 1).to(self.dev)

        pos_embs = self.feat2emb(pos_seqs, pos_feature, include_user=False,cold_start_list_tensor=cold_start_list_tensor_pos)
        neg_embs = self.feat2emb(neg_seqs, neg_feature, include_user=False,cold_start_list_tensor=cold_start_list_tensor_neg)

        # 归一化
        log_feats = F.normalize(log_feats.float(), dim=-1, eps=1e-8).to(log_feats.dtype)
        pos_embs = F.normalize(pos_embs.float(), dim=-1, eps=1e-8).to(pos_embs.dtype)
        neg_embs = F.normalize(neg_embs.float(), dim=-1, eps=1e-8).to(neg_embs.dtype)
    ...
    def predict(self, log_seqs,seq_user_feat, seq_item_feat, mask, action_type, input_interval=None,target_action_type=None,cold_start_seq_tensor=None):
        """Prediction method"""
        ...
        log_feats = self.log2feats(log_seqs, mask, seq_user_feat,seq_item_feat, input_interval, action_type, next_action_type, next_mask,cold_start_list_tensor=cold_start_seq_tensor)
        final_feat = log_feats[:, -1, :]
        final_feat = F.normalize(final_feat.float(), dim=-1, eps=1e-8).to(final_feat.dtype)
        return final_feat