...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, args):
        ...
        with open(Path(cache_dir, 'item_sparse_knn.json'), 'r') as f:
            self.cold_start_dict = json.load(f)

        with open(Path(cache_dir, 'item_rid_remap.json'), 'r') as f:
            self.item_rid_remap_dict = json.load(f)
        self.item_rid_remap = self.item_rid_remap_dict['mapping']
    ...
    def get_cold_start_knn(self, i):
        # 转化i为cid
        cid = self.indexer_i_rev.get(i, 0)
        cid = str(cid)
        is_cold_start = False
        temp_array = np.zeros(self.knn_num, dtype=np.int32)
        if cid in self.cold_start_dict:
            is_cold_start = True
            knn_rid_list = self.cold_start_dict[cid][:self.knn_num]
            # 物品重映射
            knn_rid_list = [self.item_rid_remap.get(str(rid), 0) for rid in knn_rid_list]
            knn_rid_array = np.array(knn_rid_list, dtype=np.int32)
            temp_array = knn_rid_array
        return is_cold_start, temp_array
    def __getitem__(self, uid):
        ...
        for record_tuple in reversed(input_sequence):
            ...
            if type_ == 1:
                ...
                is_cold_start, temp_array = self.get_cold_start_knn(i)
                # 标记冷启动用户
                if is_cold_start:
                    i = -1   # cold start item
                    cold_start_knn_seq.append(temp_array)
                else:
                    # 重映射id
                    i = self.item_rid_remap.get(str(i), 0)
            else:
            ...
            if next_type == 1 and next_i != 0:
                ...
                # 正样本冷启动标记
                is_cold_start, temp_array = self.get_cold_start_knn(next_i)
                if is_cold_start:
                    next_i = -1   # cold start item
                    cold_start_knn_pos.append(temp_array)
                else:
                    # 重映射id
                    next_i = self.item_rid_remap.get(str(next_i), 0)
                pos[idx] = next_i
                neg_id = self._random_neq(1, self.itemnum + 1, total_item)
                neg_feat_i = deepcopy(self.item_feat_dict.get(str(neg_id), {}))
                neg_feat[idx] = self.fill_missing_feat(neg_feat_i, neg_id,is_user=False)
                # 负样本冷启动标记
                is_cold_start, temp_array = self.get_cold_start_knn(neg_id)
                if is_cold_start:
                    neg_id = -1   # cold start item
                    cold_start_knn_neg.append(temp_array)
                else:
                    # 重映射id
                    neg_id = self.item_rid_remap.get(str(neg_id), 0)
                neg[idx] = neg_id
            nxt = record_tuple
        ...
        return seq, pos, neg, token_type, action_type, next_token_type, next_action_type, seq_feat_user, \
        seq_feat_item, pos_feat, neg_feat, cur_timestamp, next_timestamp, cold_start_knn_seq, cold_start_knn_pos, cold_start_knn_neg
    ...
    def collate_fn(self, batch):
        ...
        seq, pos, neg, token_type, action_type, next_token_type, next_action_type, seq_feat_user, \
        seq_feat_item, pos_feat, neg_feat, cur_timestamp, next_timestamp,cold_start_knn_seq, cold_start_knn_pos, \
        cold_start_knn_neg = zip(*batch)
        cur_timestamp = torch.from_numpy(np.array(cur_timestamp))
        next_timestamp = torch.from_numpy(np.array(next_timestamp))

        cold_start_knn_list_seq = []
        cold_start_knn_list_pos = []
        cold_start_knn_list_neg = []

        for i in range(len(cold_start_knn_seq)):
            if len(cold_start_knn_seq[i]) > 0:
                cold_start_knn_list_seq.extend(cold_start_knn_seq[i])
            if len(cold_start_knn_pos[i]) > 0:
                cold_start_knn_list_pos.extend(cold_start_knn_pos[i])
            if len(cold_start_knn_neg[i]) > 0:
                cold_start_knn_list_neg.extend(cold_start_knn_neg[i])
        cold_start_knn_tensor_seq = torch.from_numpy(np.array(cold_start_knn_list_seq))
        cold_start_knn_tensor_pos = torch.from_numpy(np.array(cold_start_knn_list_pos))
        cold_start_knn_tensor_neg = torch.from_numpy(np.array(cold_start_knn_list_neg))
        ...
        return (seq, pos, neg, token_type, action_type, next_token_type, next_action_type,
                ...
                cur_timestamp, next_timestamp,cold_start_knn_tensor_seq, cold_start_knn_tensor_pos, cold_start_knn_tensor_neg)
...
class MyTestDataset(MyDataset):
    ...
    def __getitem__(self, uid):
        ...
        if self.load_time_intervals:
            ...
            return seq, token_type, action_type, seq_user_feat, seq_item_feat, user_id, cur_timestamp, target_act_type, cold_start_knn_seq
        else:
    ...
    def collate_fn(self, batch):
        ...
        seq, token_type, action_type, seq_feat_user, seq_feat_item, user_id, cur_timestamp,target_actions, cold_start_knn_seq = zip(*batch)
        cur_timestamp = torch.from_numpy(np.array(cur_timestamp))
        ...
        return seq, token_type, action_type, seq_feat_user_tensors, seq_feat_item_tensors, user_id, cur_timestamp, target_actions, cold_start_knn_tensor_seq
