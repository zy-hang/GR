...
def train_model(args, log_file, writer, data_path):
    ...
    remap_item_num = dataset.item_rid_remap_dict['new_itemnum']
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step, batch in enumerate(train_loader):
            seq, pos, neg, token_type, action_type, next_token_type, next_action_type, \
            seq_feat_user,seq_feat_item, pos_feat, neg_feat, cur_time, next_time,cold_start_list_tensor_seq,\
            cold_start_list_tensor_pos, cold_start_list_tensor_neg = batch
            def to_dev(d):
            ...
            with autocast(dtype=torch.bfloat16, device_type='cuda', enabled=True):
                pos_logits, _, neg_bank_tuple = model(
                    ...
                    cold_start_list_tensor_seq, cold_start_list_tensor_pos, cold_start_list_tensor_neg
                )
            ...
            loss, loss_click, lose_expose, pos_mean_click, neg_mean_click, pos_mean_all, neg_mean_all = get_loss_streaming(
                args, pos_logits, (q_valid, neg_cache, model, neg_bank_cur), next_token_type, next_action_type, action_only=False
            )
            if args.l2_emb > 0:
            ...
            if neg_cache is not None:
                with torch.no_grad():
                    if args.neg_cache_on_cpu:
                        ...
                        cold_start_list_tensor_neg = cold_start_list_tensor_neg.detach()
                    neg_cache.append((neg_store, neg_feat_store, next_token_type_store,cold_start_list_tensor_neg))
