...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, args):
        ...
        self._build_buckets() # 初始化物品分桶，用于负采样
    ...
    def _sample_neg_bucketed(self, avoid_set: set, try_max: int = 8) -> int:
        for _ in range(try_max):
            b = np.random.choice(len(self.bucket_items), p=self.bucket_weights)
            pool = self.bucket_items[b]
            if pool.size == 0:
                continue
            t = int(pool[np.random.randint(0, pool.size)])
    ...
    def _random_neq(self, l, r, s):
        ...
        Returns:
            t: 不在序列s中的随机整数
        """
        # s: avoid_set（用户历史/正例等）
        if 1:  # hasattr(self, "bucket_items"):
            return self._sample_neg_bucketed(avoid_set=s)
    ...
    def __getitem__(self, uid):
        ...
        for record_tuple in reversed(input_sequence):
            ...
            if next_type == 1 and next_i != 0:
                ...
                # 使用重映射表进行ID转换
                # neg_id = self.item_expose[str(neg_id)]
                neg[idx] = neg_id
            nxt = record_tuple
    ...
    # ---- 一次性建桶 ----
    def _build_buckets(self, expo_counts=None, bucket_weights=None, bank_target=None):
        """
        构建按曝光数分桶的物品池:
        桶0: x<=3
        桶1: 3<x<=9
        ...
        桶4: 100<x<=1000
        桶5: 1000<x
        优先读取 USER_CACHE_PATH/item_ctr_feat_count.json 中的 '301'(曝光计数)。
        若无该文件则回退到单次遍历 _load_user_data 统计曝光。
        """
        import os, json
        import numpy as np

        B = 6
        self.bucket_items = [[] for _ in range(B)]

        # -------- 1) 准备每个 item 的“曝光数” --------
        EXP_FEAT_ID = '301'  # 与 genCtrFeature 写文件时的一致：'301' 表示曝光原始计数

        if expo_counts is None:
            # 1.1 优先读缓存文件（由 analyze_time_and_ctr_features() 生成）
            cache_dir = os.environ.get('USER_CACHE_PATH', '.')
            ctr_path = os.path.join(cache_dir, "item_ctr_feat_count.json")
            expo_arr = None
            if os.path.exists(ctr_path):
                with open(ctr_path, "r", encoding="utf-8") as f:
                    ctr_map = json.load(f)  # { "item_id(str)": { "301": expose, "302": click, ... } }
                expo_arr = np.zeros(self.itemnum + 1, dtype=np.int32)
                # JSON 的 key 是字符串 item_id，值里取 '301' 字段
                for k, v in ctr_map.items():
                    iid = int(k)
                    if 0 < iid <= self.itemnum:
                        expo_arr[iid] = int(v.get(EXP_FEAT_ID, 0))

            expo_counts = expo_arr
        else:
            # 传进来的是 dict 或 ndarray 都可
            if isinstance(expo_counts, dict):
                arr = np.zeros(self.itemnum + 1, dtype=np.int32)
                for iid, c in expo_counts.items():
                    iid_int = int(iid)
                    if 0 < iid_int <= self.itemnum:
                        arr[iid_int] = int(c)
                expo_counts = arr
            else:
                expo_counts = np.asarray(expo_counts, dtype=np.int32)

        # -------- 2) 分桶边界（与 genCtrFeature.bucket_expose_count 完全一致） --------
        def bucket_idx(x: int) -> int:
            if x <= 3:       return 0
            elif x <= 9:     return 1
            elif x <= 54:    return 2
            elif x <= 100:   return 3
        ...
        # -------- 3) 过滤无特征的 item（若你这边 item_feat_dict 使用 str(iid) 作键则保持一致） --------
        def has_feat(iid: int) -> bool:
            return (not getattr(self, "item_feat_dict", None)) or (str(iid) in self.item_feat_dict)

        for iid in range(1, self.itemnum + 1):
            if not has_feat(iid):
                continue
            b = bucket_idx(int(expo_counts[iid]))
            self.bucket_items[b].append(iid)

        self.bucket_items = [np.array(x, dtype=np.int32) for x in self.bucket_items]

        # -------- 4) 桶权重 --------
        if bucket_weights is None:
            w = np.array([0.453, 0.196, 0.202, 0.048, 0.082, 0.019], dtype=np.float64) # 0.251, 0.265, 0.274, 0.068, 0.113, 0.029 # 0.453, 0.196, 0.202, 0.048, 0.082, 0.019   # 0.333, 0.238, 0.246, 0.059, 0.098, 0.026
        else:
            w = np.array(bucket_weights, dtype=np.float64)

        # 动态限顶（最热桶），降低大负库撞样率（可选）
        # top = 5
        # N_top = len(self.bucket_items[top])
        # if N_top > 0 and bank_target is not None:
        #     cap = N_top / max(4 * bank_target, 1)  # 头部可用量/抽样量的 4 倍
        #     w[top] = min(w[top], cap)

        # 空桶置 0 并重归一
        mask = np.array([len(b) > 0 for b in self.bucket_items])
        w[~mask] = 0.0
        w = w / w.sum()
        self.bucket_weights = w
