...
class MyDataset(torch.utils.data.Dataset):
    ...
    def _build_buckets(self, expo_counts=None, bucket_weights=None, bank_target=None):
        ...
        if expo_counts is None:
            ...
            # 1.2 回退方案：扫一遍数据集按 tuple 结构统计曝光（i 和 item_feat 同时存在才计）
            # if expo_arr is None:
            #     expo_arr = np.zeros(self.itemnum + 1, dtype=np.int32)
        ...
        if bucket_weights is None:
            w = np.array([0.251, 0.265, 0.274, 0.068, 0.113, 0.029], dtype=np.float64) # 0.453, 0.196, 0.202, 0.048, 0.082, 0.019 #涨0.0015
        else:
