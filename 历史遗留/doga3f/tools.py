...
def get_args():
    ...
    parser.add_argument('--neg_cache_steps', type=int, default=2,
    ...
    # 添加负样本数
    parser.add_argument('--num_neg', default=5, type=int, help='每个正样本采样的负样本数')
...
def get_loss_streaming(args, pos_id, neg_id, pos_logits, neg_bank_tuple,
                       next_token_type, next_action_type,
                       action_only=False, temperature=0.025):
    
    mask_all   = (next_token_type == 1)
    mask_expose = (next_token_type == 1) & (next_action_type == 0)
    ...
    expose_mask_valid = mask_expose[mask_all]

    q_valid, neg_cache, model, neg_bank_cur = neg_bank_tuple
    q_vaild_id = pos_id[mask_all] # 用于去除负样本与正样本ID重复的部分，形状为[M]
    B,L,N = neg_id.shape
    neg_id_vaild = neg_id.permute(0,2,1).reshape(B*N, L)
    mask_flat_2d_cur = mask_all.unsqueeze(1).expand(-1, N, -1).reshape(B*N, L)
    neg_id_vaild = neg_id_vaild[mask_flat_2d_cur]  # [*,]
    all_remove_ids_cnt = 0
    all_neg_ids_cnt = 0
    scale = pos_logits.new_tensor(1.0 / float(temperature), dtype=torch.float32)  

    pos_all_valid = pos_logits[mask_all].to(torch.float32)

    # 安全均值，避免空集合 NaN；如需与训练信号完全一致，可改为在减 margin 后再统计
    def safe_mean(x):
        return x.mean() if x.numel() > 0 else x.new_tensor(0.0)

    pos_sim_expose = safe_mean(pos_all_valid[expose_mask_valid])
    pos_sim_click  = safe_mean(pos_all_valid[click_mask_valid])
    pos_sim_conv   = safe_mean(pos_all_valid[conv_mask_valid])
    ...
    # ====== 单遍流式 log-sum-exp + 负样本相似度的“总和/总对数”汇总 ======
    lse = s_pos.clone().to(torch.float32)

    # 用标量张量做全局累加，避免等权平均带来的偏差
    neg_sum_expose = lse.new_zeros(())
    neg_sum_click  = lse.new_zeros(())
    neg_sum_conv   = lse.new_zeros(())
    neg_cnt_expose = lse.new_zeros(())
    neg_cnt_click  = lse.new_zeros(())
    neg_cnt_conv   = lse.new_zeros(())

    chunk = 4096
    m_ex, m_clk, m_cnv = expose_mask_valid, click_mask_valid, conv_mask_valid

    def accumulate_sims(s_raw):
        nonlocal neg_sum_expose, neg_sum_click, neg_sum_conv
        nonlocal neg_cnt_expose, neg_cnt_click, neg_cnt_conv
        K = s_raw.size(1)  # 当前块的负样本数
        if m_ex.any():
            neg_sum_expose = neg_sum_expose + s_raw[m_ex].sum()
            neg_cnt_expose = neg_cnt_expose + m_ex.sum().to(s_raw.dtype) * K
        if m_clk.any():
            neg_sum_click  = neg_sum_click  + s_raw[m_clk].sum()
            neg_cnt_click  = neg_cnt_click  + m_clk.sum().to(s_raw.dtype) * K
        if m_cnv.any():
            neg_sum_conv   = neg_sum_conv   + s_raw[m_cnv].sum()
            neg_cnt_conv   = neg_cnt_conv   + m_cnv.sum().to(s_raw.dtype) * K

    # 处理历史 cache（确保设备与归一化一致）
    if neg_cache is not None and len(neg_cache) > 0:
        for neg_cached, neg_feat_cached, next_token_type_cached in neg_cache:
            ...
            neg_ids_flat_valid = neg_ids_flat[mask_flat_2d]    # [*]，用于去除与正样本ID重复的部分
            all_neg_ids_cnt += neg_ids_flat_valid.numel()
            for st in range(0, bank.size(0), chunk):
                bk = bank[st:st + chunk, :]
                neg_ids_flat_valid_chunk = neg_ids_flat_valid[st:st + chunk]
                s = (q_valid @ bk.T).to(torch.float32)
                accumulate_sims(s)
                s = s* scale
                # 去除与正样本ID重复的部分
                mask_same = (q_vaild_id.view(-1, 1) == neg_ids_flat_valid_chunk.view(1, -1))  # [M, chunk_size]
                all_remove_ids_cnt += mask_same.sum().item()
                s = s.masked_fill(mask_same, float('-inf'))
                lse = torch.logaddexp(lse, torch.logsumexp(s, dim=1))
                del s, bk, mask_same, neg_ids_flat_valid_chunk
            del bank
    ...
    all_neg_ids_cnt += neg_id_vaild.numel()
    for st in range(0, bank.size(0), chunk):
        bk = bank[st:st + chunk, :]
        neg_ids_flat_valid_chunk = neg_id_vaild[st:st + chunk]
        s = (q_valid @ bk.T).to(torch.float32)
        accumulate_sims(s)
        s = s * scale
        # 去除与正样本ID重复的部分
        mask_same_id = (q_vaild_id.view(-1, 1) == neg_ids_flat_valid_chunk.view(1, -1))  # [M, num_neg]
        all_remove_ids_cnt += mask_same_id.sum().item()
        s = s.masked_fill(mask_same_id, float('-inf'))
        lse = torch.logaddexp(lse, torch.logsumexp(s, dim=1))
        del s, bk, mask_same_id, neg_ids_flat_valid_chunk

    # 负样本平均相似度（安全除法；与 pos 相同尺度：未除温度）
    neg_sim_expose = neg_sum_expose / torch.clamp(neg_cnt_expose, min=1.0)
    neg_sim_click  = neg_sum_click  / torch.clamp(neg_cnt_click,  min=1.0)
    neg_sim_conv   = neg_sum_conv   / torch.clamp(neg_cnt_conv,   min=1.0)

    del bank, q_valid, q_vaild_id, neg_id_vaild
    torch.cuda.empty_cache()
    ...
    return loss, (all_remove_ids_cnt, all_neg_ids_cnt),pos_sim_expose, pos_sim_click, pos_sim_conv, neg_sim_expose, neg_sim_click, neg_sim_conv
...
def output_info(global_step, writer,output_dict,is_train):
