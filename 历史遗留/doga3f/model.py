...
class HSTUModel(nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):
        ...
        # item Embedding
        self.item_emb = nn.Embedding(self.item_num + 1, 64, padding_idx=0, dtype=torch.bfloat16)
        self.item_emb_ = nn.Embedding(self.item_num + 1, 64, padding_idx=0, dtype=torch.bfloat16)
    ...
    def forward(
        ...
        N = neg_seqs.shape[2]                          # num_neg
        neg_flat_ids = neg_seqs.permute(0, 2, 1).reshape(B * N, L)   # [B*N, L]
        neg_embs = self.feat2emb(neg_flat_ids, neg_feature, include_user=False)  # [B*N, L, D]
        neg_embs = F.normalize(neg_embs.float(), dim=-1, eps=1e-8).to(neg_embs.dtype)
        # 只保留可预测位 → 2D neg_bank
        mask_flat = loss_mask.unsqueeze(1).expand(B, N, L).reshape(B * N, L)      # [B*N, L]
        D = neg_embs.size(-1)
        neg_bank = neg_embs[mask_flat].view(-1, D)                                # [*, D]
